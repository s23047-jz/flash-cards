      ...
      <Row className="w-full">
        ...
        <Row className="h-full" style={styles.cardRows}>
          ...
          <Col className="w-full">
            ...
            <Text className="text-center font-bold">{downloads}</Text>
          </Col>
        </Row>
        <Row className="h-full" style={styles.cardRows}>
...
const UserCard: React.FC<UserListInterface> = ({
  id,
  rank,
  username,
  shared,
      ...
      <Card className="w-full h-full">
        <Row className="w-full">
          ...
          <Row className="h-full" style={styles.cardRows}>
            ...
            <Col className="w-full">
              <Text className="text-center">Shared decks</Text>
              <Text className="text-center font-bold">{shared}</Text>
            </Col>
          </Row>
        </Row>
      </Card>
...
const DeckList: React.FC<ScreenProps> = ({ navigation, route }) => {
  ...
  const fetchDecks = async () => {
    ...
    if (data && data.length)
      setData((prevData) => [...prevData, ...deck_list.decks]);
  ...
  const fetchUsers = async () => {
    const user_list = await UsersService.getUsersRanking(
      usersQuery,
      navigation,
    );
    if (data && data.length)
      setData((prevData) => [...prevData, ...user_list.users]);
    ...
    <View className="flex h-screen w-full bg-sky-500 dark:bg-blue-900">
      <View className="flex flex-container w-full mt-20 mb-5">
        <Row className="w-full p-6" style={styles.row}>
          <Col className="w-full h-10">
            <Text className="text-2xl text-white font-bold text-right">
              {selectedView === PAGES.DECKS ? "Decks ranking" : "Users ranking"}
            </Text>
          </Col>
        </Row>
        ...
        <Row className="w-full mt-3" style={styles.row}>
          ...
          >
            <Button
              style={styles.button}
              className={`p-4 ${selectedView === PAGES.DECKS ? "bg-sky-300" : "bg-sky-700 border-black"}`}
              onPress={async () => changeView(PAGES.DECKS)}
            >
              <Text className="text-lg ml-auto mr-auto font-bold">Decks</Text>
            </Button>
          </Col>
          <Col
            className="h-full justify-center items-center"
          ...
          >
            <Button
              style={styles.button}
              className={`p-4 ${selectedView === PAGES.USERS ? "bg-sky-300" : "bg-sky-700 border-black"}`}
              onPress={async () => changeView(PAGES.USERS)}
            >
              <Text className="text-lg ml-auto mr-auto font-bold">Users</Text>
            </Button>
          </Col>
        </Row>
        <View className="h-screen pb-28">
